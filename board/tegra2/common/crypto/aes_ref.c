...
static void shift_rows       (NvU8 *state);
static void inv_shift_rows    (NvU8 *state);
static void mix_sub_columns   (NvU8 *state);
static void inv_mix_sub_columns(NvU8 *state);
static void add_round_key     (NvU32 *state, NvU32 *key);

static NvU8 s_Sbox[256] =
{	/* forward s-box */
 ...
 */
void
inv_shift_rows(NvU8 *state)
{
	NvU8 tmp;

	/* restore row 0 */
	state[ 0] = s_InvSbox[state[ 0]];
	state[ 4] = s_InvSbox[state[ 4]];
	state[ 8] = s_InvSbox[state[ 8]];
	state[12] = s_InvSbox[state[12]];

	/* restore row 1 */
	tmp       = s_InvSbox[state[13]];
	state[13] = s_InvSbox[state[ 9]];
	state[ 9] = s_InvSbox[state[ 5]];
	state[ 5] = s_InvSbox[state[ 1]];
	state[ 1] = tmp;

	/* restore row 2 */
	tmp       = s_InvSbox[state[ 2]];
	state[ 2] = s_InvSbox[state[10]];
	state[10] = tmp;
	tmp       = s_InvSbox[state[ 6]];
	state[ 6] = s_InvSbox[state[14]];
	state[14] = tmp;

	/* restore row 3 */
	tmp       = s_InvSbox[state[ 3]];
	state[ 3] = s_InvSbox[state[ 7]];
	state[ 7] = s_InvSbox[state[11]];
	state[11] = s_InvSbox[state[15]];
	state[15] = tmp;
}

/* recombine and mix each row in a column */
void
mix_sub_columns(NvU8 *state)
{
	NvU8 tmp[4 * NVAES_STATECOLS];
...
void inv_mix_sub_columns (NvU8 *state)
{
	NvU8 tmp[4 * NVAES_STATECOLS];
	int  i;

	/* restore column 0 */
	tmp[ 0] = s_XtimeE[state[ 0]] ^ s_XtimeB[state[ 1]] ^
			s_XtimeD[state[ 2]] ^ s_Xtime9[state[ 3]];
	tmp[ 5] = s_Xtime9[state[ 0]] ^ s_XtimeE[state[ 1]] ^
			s_XtimeB[state[ 2]] ^ s_XtimeD[state[ 3]];
	tmp[10] = s_XtimeD[state[ 0]] ^ s_Xtime9[state[ 1]] ^
			s_XtimeE[state[ 2]] ^ s_XtimeB[state[ 3]];
	tmp[15] = s_XtimeB[state[ 0]] ^ s_XtimeD[state[ 1]] ^
			s_Xtime9[state[ 2]] ^ s_XtimeE[state[ 3]];

	/* restore column 1 */
	tmp[ 4] = s_XtimeE[state[ 4]] ^ s_XtimeB[state[ 5]] ^
			s_XtimeD[state[ 6]] ^ s_Xtime9[state[ 7]];
	tmp[ 9] = s_Xtime9[state[ 4]] ^ s_XtimeE[state[ 5]] ^
			s_XtimeB[state[ 6]] ^ s_XtimeD[state[ 7]];
	tmp[14] = s_XtimeD[state[ 4]] ^ s_Xtime9[state[ 5]] ^
			s_XtimeE[state[ 6]] ^ s_XtimeB[state[ 7]];
	tmp[ 3] = s_XtimeB[state[ 4]] ^ s_XtimeD[state[ 5]] ^
			s_Xtime9[state[ 6]] ^ s_XtimeE[state[ 7]];

	/* restore column 2 */
	tmp[ 8] = s_XtimeE[state[ 8]] ^ s_XtimeB[state[ 9]] ^
			s_XtimeD[state[10]] ^ s_Xtime9[state[11]];
	tmp[13] = s_Xtime9[state[ 8]] ^ s_XtimeE[state[ 9]] ^
			s_XtimeB[state[10]] ^ s_XtimeD[state[11]];
	tmp[ 2] = s_XtimeD[state[ 8]] ^ s_Xtime9[state[ 9]] ^
			s_XtimeE[state[10]] ^ s_XtimeB[state[11]];
	tmp[ 7] = s_XtimeB[state[ 8]] ^ s_XtimeD[state[ 9]] ^
			s_Xtime9[state[10]] ^ s_XtimeE[state[11]];

	/* restore column 3 */
	tmp[12] = s_XtimeE[state[12]] ^ s_XtimeB[state[13]] ^
			s_XtimeD[state[14]] ^ s_Xtime9[state[15]];
	tmp[ 1] = s_Xtime9[state[12]] ^ s_XtimeE[state[13]] ^
			s_XtimeB[state[14]] ^ s_XtimeD[state[15]];
	tmp[ 6] = s_XtimeD[state[12]] ^ s_Xtime9[state[13]] ^
			s_XtimeE[state[14]] ^ s_XtimeB[state[15]];
	tmp[11] = s_XtimeB[state[12]] ^ s_XtimeD[state[13]] ^
			s_Xtime9[state[14]] ^ s_XtimeE[state[15]];

	for (i=0; i < 4 * NVAES_STATECOLS; i++)
		state[i] = s_InvSbox[tmp[i]];
}
...
void
add_round_key(NvU32 *state, NvU32 *key)
{
	int idx;
...
void
nv_aes_expand_key(NvU8 *key, NvU8 *expkey)
{
	NvU8  tmp0, tmp1, tmp2, tmp3, tmp4;
	NvU32 idx;
...
void
nv_aes_encrypt(NvU8 *in, NvU8 *expkey, NvU8 *out)
{
	NvU8  state[NVAES_STATECOLS * 4];
	NvU32 round;

	memcpy(state, in, NVAES_STATECOLS * 4);
	add_round_key((NvU32 *)state, (NvU32 *)expkey);
...
void
nv_aes_decrypt(NvU8 *in, NvU8 *expkey, NvU8 *out)
{
	NvU8  state[NVAES_STATECOLS * 4];
	NvU32 round;
	...
	add_round_key((NvU32 *)state,
			(NvU32 *)expkey + NVAES_ROUNDS * NVAES_STATECOLS);
	inv_shift_rows(state);

	for (round = NVAES_ROUNDS; round--; ) {
		add_round_key((NvU32 *)state, (NvU32 *)expkey +
						round * NVAES_STATECOLS);
		if (round)
