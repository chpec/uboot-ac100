...
#else /*USE_PROC_COMM is defined */
uint32_t rate;

    /*
     * Get the clock source for the UART from proc_comm
     */
    /*
     * Determine if TCXO (19.2MHz) or TCXO/4 (4.8MHz) is the UART clock source.
     * Use recommended m,n,d,mnd register settings.
    ...
    rate = proc_comm_get_uart_clk(UART_BASE);
    switch (rate)
    {
    case (CONFIG_TCXO_HZ):  // TCXO clock source
        mreg_val = 0x0006;
        nreg_val = 0x00F1;
        dreg_val = 0x000F;
        mndreg_val = 0x001A;
    ...
    case (CONFIG_TCXO_HZ / 4):  // TCXO/4 clock source
        mreg_val = 0x0018;
        nreg_val = 0x00F6;
        dreg_val = 0x000F;
        mndreg_val = 0x000A;
